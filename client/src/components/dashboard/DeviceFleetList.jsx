export function DeviceFleetList({
  ...
  return <div className="space-y-4 mb-6">
      ...
      <div className="flex flex-col gap-3">
        ...
        return <div key={device.deviceId} onClick={() => setSelectedDeviceId(device.deviceId)} className="relative p-4 rounded-xl transition-all duration-300 cursor-pointer overflow-hidden flex flex-col sm:flex-row sm:items-center justify-between gap-4" style={{
              ...
              border: '1px solid rgba(71,85,105,0.55)'
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    {(health.lowNetwork || health.connQuality) && <span className="text-[10px] text-slate-400 font-medium">📶 {health.lowNetwork ? 'LOW BT' : health.connQuality?.toUpperCase() || 'OK'}</span>}
                  </div>
                </div>
              </div>

              {isSelected && <div className="absolute left-0 top-0 bottom-0 w-1 rounded-l-xl bg-gradient-to-b from-cyan-400 to-emerald-400" />}
            </div>;
      })}
      </div>
    </div>;
