...
type DeviceListProps = {
  ...
  onListen: (deviceId: string) => void
  onStop: (deviceId: string) => void
  onTakePhoto: (deviceId: string) => void
  onForceReconnect: (deviceId: string) => void
  onForceUpdate: (deviceId: string) => void
}

function formatLastSeen(lastTs?: number | null) {
  if (!lastTs) return '—'
  try {
    return new Date(lastTs).toLocaleTimeString([], { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' })
  } catch {
    return '—'
  }
}
...
export const DeviceList = memo(function DeviceList({
  ...
  return (
    <div className="space-y-4">
      {sorted.length === 0 ? (
        <div className="rounded-xl border border-dark-700 bg-dark-800/40 p-6 text-center text-slate-400">
        ...
        sorted.map((d) => {
          ...
          return (
            <div
              key={d.deviceId}
              className="rounded-xl border border-dark-700 bg-dark-800/40 p-4 shadow-card"
              style={{
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex flex-col gap-2 w-[240px] shrink-0">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    <button
                      onClick={() => onTakePhoto(d.deviceId)}
                      disabled={false}
                      className="rounded-xl px-3 py-2 text-xs font-semibold transition-colors"
                      style={{
                    ...
                    <button
                      onClick={() => onForceReconnect(d.deviceId)}
                      disabled={false}
                      className="rounded-xl px-3 py-2 text-xs font-semibold transition-colors"
                      style={{
                    ...
                    <button
                      onClick={() => onForceUpdate(d.deviceId)}
                      disabled={false}
                      className="rounded-xl px-3 py-2 text-xs font-semibold transition-colors"
                      style={{
