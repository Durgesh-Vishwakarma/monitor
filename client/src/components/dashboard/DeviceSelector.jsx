...
export function DeviceSelector({
  ...
  return <Panel title="Device Selector" subtitle="Select a device to control">
      {/* Device Cards */}
      <div className="mb-3 grid gap-2 sm:grid-cols-2">
        {devices.length === 0 ? <div className="col-span-2 rounded border border-dark-700 bg-dark-900/50 p-4 text-center text-sm text-slate-400">
            No devices connected
          </div> : devices.map(device => <button key={device.deviceId} onClick={() => onSelect(device.deviceId)} className={`
                rounded border p-3 text-left transition-colors
                ${selectedDeviceId === device.deviceId ? 'border-blue-500 bg-blue-500/10' : 'border-dark-700 bg-dark-900/50 hover:border-dark-600'}
              `.trim()}>
